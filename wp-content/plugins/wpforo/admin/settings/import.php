<form method="post" onsubmit="return !!document.querySelector('.wpf-opt-row-imp-exp .wpf-opt-checkbox-wrap input:checked')" enctype="multipart/form-data">
    <input type="hidden" name="wpfaction" value="settings_import">
	<?php wp_nonce_field( "wpforo_settings_import" ); ?>
    ...
    <div class="wpf-opt-row">
        <div class="wpf-opt-intro">
			...
			<?php _e( 'It allows users to import the configuration and settings data of the wpForo from already exported JSON file.', 'wpforo' ) ?>
        </div>
    </div>
    ...
    <div class="wpf-subtitle">
        <span class="dashicons dashicons-admin-links"></span> <?php _e( 'Choose to Upload JSON file.', 'wpforo' ) ?>
    </div>

    <div class="wpf-opt-row wpf-opt-row-imp-exp">
        <div class="wpf-opt-input">
            <input type="file" name="wpfsimpjson" accept=".json" required/>
        </div>
    </div>
    ...
    <div class="wpf-opt-row" style="display: flex; justify-content: space-between; align-items: center;">
        ...
        <input type="submit" value="<?php _e( 'Import JSON File', 'wpforo' ) ?>" class="button-primary" title="<?php _e( 'One or more checkboxes must be selected.', 'wpforo' ) ?>">
    </div>
</form>
